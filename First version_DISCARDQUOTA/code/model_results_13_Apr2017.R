...
#----------------------------------------------------------------------------------
catchmean_2012        <- melt(lapply(catchmean, '[',j=10))
catchmean_2012$effort <- rep(melt(effort_days[,as.character(2012)]%/%number_vessels[,as.character(2012)])$value,each=2)

catchmean_2012$cpue   <- (catchmean_2012$value/catchmean_2012$effort)*1000
catchmean_2012$area   <- with(catchmean_2012, paste(area, unit, sep=" "))
names(catchmean_2012) <- c("cat","year", "unit", "season", "area", "iter", "catch", "spp", "effort", "cpue" )
catchmean_2012        <- subset(catchmean_2012, select= c("cat", "season","area","catch", "spp", "effort", "cpue"))

catchmean_2012$spp    <- as.factor(catchmean_2012$spp)
levels(catchmean_2012$spp) <- c(levels(catchmean_2012 $spp)," horse\n mackerel")
catchmean_2012$spp[catchmean_2012$spp=="horse_mackerel"] <- c(" horse\n mackerel")

catchmean_2012        <- subset(catchmean_2012, !(area %in% c("VI PTB", "VII PTB")))

# replace discards the others for just discards withpout MCRS
#-------------------------------------------------------------
a<- subset(catchmean_2012, spp %in% "others" & cat %in% c('< MCRS'))
a$cat<- 'discards'
b<- subset(catchmean_2012, spp %in% "others" & cat %in% c('> MCRS'))
b$cat<- 'landings'

catchmean_2012 <-subset(catchmean_2012, !(spp %in% "others" & cat %in% c('< MCRS','> MCRS')))
catchmean_2012 <- rbind(catchmean_2012, a, b)
rm(a, b)

catchmean_2012$cat    <- factor(catchmean_2012$cat, levels=c("> MCRS","< MCRS","landings","discards"))
catchmean_2012$area   <- factor(catchmean_2012$area, levels=c("VI OTB","VII OTB","VIIIabd OTB","VIIIabd PTB"))
catchmean_2012$spp    <- factor(catchmean_2012$spp, levels=c("hake","megrim","mackerel"," horse\n mackerel","others"))

#catchmean_model[is.na(catchmean_model)]<-0
catchmean_2012 [is.na(catchmean_2012)] <- 0
catchmean_2012$catch[catchmean_2012$area=="VII OTB"] <- NA
...
#-----------------------------------------------------------------------------
# Differences between the mean and the 2012 data (grey 2012, black: mean)
png(filename="~/Dropbox/BoB/DynState/2017/figures/cpue_mean_2012_logscale.png",
    width=12, height=11, units="cm", res=500, pointsize=8)

ggplot() +  
  geom_line (data= catchmean_2012, aes(x =factor(season), y =pmax(catch,0.01), group=cat, shape=cat, linetype=cat ),size=0.25, colour="blue")+
  geom_point(data= catchmean_2012, aes(x =factor(season), y =pmax(catch,0.01), group=cat, shape=cat, linetype=cat) ,size=1, colour="blue")+
  ...
  #scale_colour_manual(name='Dataset', labels = c("2012 CPUE","Average CPUE (2003-2012 period)"), values=c('blue', 'black'), guide='legend') +
