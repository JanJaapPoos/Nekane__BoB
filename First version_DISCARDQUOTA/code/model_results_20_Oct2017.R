...
#------------------------------------------------------------------------                                                          
random<- c(100,52, 98,24,101 ,129 ,197, 200,  1, 146, 174,  87, 190,  49,  64) #58 where is 1
...
data3 <- as.matrix(dapply(effort_res_15simul_df$effort, effort_res_15simul_df$scenario, FUN=sum))
...
effort_res_modelval_df <- subset(effort_res_15simul_df, (scenario %in% c("2012","Model validation")))
# Remove mean effort or 2012 effort and model validation
effort_res_15simul_df <- subset(effort_res_15simul_df, !(scenario %in% c("2012","Model validation")))
#effort_res_15simul_df <- rbind(effort_res_15simul_df, ref_effort[,1:6])

#JUST FOR PLOTTING
#rename scenarios for plotting the total values of effort in the header , paste("n =",data3[1])
modeffort <- effort_res_modelval_df
modeffort$scenario       <- factor(modeffort$scenario,levels=c("2012","Model validation", "2012 \n (n=3338)","Model validation \n (n=3705)"))
modeffort$scenario[modeffort$scenario=="2012"]<- "2012 \n (n=3338)" # \n (n=data3[1])"
modeffort$scenario[modeffort$scenario=="Model validation"]<- "Model validation \n (n=3705)"
modeffort$scenario       <- factor(modeffort$scenario,levels=c("2012 \n (n=3338)","Model validation \n (n=3705)"))
modeffort
...
effortscen <- effort_res_15simul_df
effortscen$scenario       <- factor(effortscen$scenario,levels=c("Scenario I","Scenario II", "Scenario III", "Scenario IV",
                                                                 "Scenario I \n (n=3361)","Scenario II \n (n=2160)", "Scenario III \n (n=1185)", "Scenario IV \n (n=2961)" ))
effortscen$scenario[effortscen$scenario=="Scenario I"]<- "Scenario I \n (n=3361)"
effortscen$scenario[effortscen$scenario=="Scenario II"]<- "Scenario II \n (n=2160)"
effortscen$scenario[effortscen$scenario=="Scenario III"]<- "Scenario III \n (n=1185)"
effortscen$scenario[effortscen$scenario=="Scenario IV"]<- "Scenario IV \n (n=2961)"
effortscen$scenario       <- factor(effortscen$scenario,levels=c("Scenario I \n (n=3361)","Scenario II \n (n=2160)", "Scenario III \n (n=1185)", "Scenario IV \n (n=2961)"))

#==============================================================================
png(filename="~/Dropbox/BoB/DynState/2017/figures/effort_area_gear_15_SIMUL.png",
...
png(filename="~/Dropbox/BoB/DynState/2017/figures/effort_area_gear_MODELVAL.png",
             width=15, height=6, units="cm", res=500, pointsize=8)
