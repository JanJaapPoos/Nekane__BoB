...
validDynState.control <- function(object){
	...
	if (object@spp1DisQuota <= 0)
		return ("value of spp1DisQuota must be > 0")
        if (object@spp2DisQuota <= 0)
		  return("value of spp2DisQuota must be > 0")
...
setClass("DynState.control",
	representation(
        ...
        spp1DisQuota          ="numeric",
        spp2DisQuota          ="numeric",
        spp1LndQuotaFine      ="numeric",
        spp2LndQuotaFine      ="numeric",
        spp1DisQuotaFine      ="numeric",
        spp2DisQuotaFine      ="numeric",
        simNumber             ="integer",
        fuelUse               ="numeric",
        fuelPrice             ="numeric",
	...
	prototype=prototype(
        ...
        spp1DisQuota          =as.double(8),
        spp2DisQuota          =as.double(8),
        spp1LndQuotaFine      =as.double(1e8),
        spp2LndQuotaFine      =as.double(1e8),
        spp1DisQuotaFine      =as.double(1e8),
        spp2DisQuotaFine      =as.double(1e8),
 	...
 	landingCosts          =as.double(0), 
        ...
        interpolationdistance =as.integer(8),
...
DynState.control <- function(spp1LndQuota=4000, spp2LndQuota=4000, spp1DisQuota=4000, spp2DisQuota=4000, spp1LndQuotaFine = 1E8, spp2LndQuotaFine=1E8, spp1DisQuotaFine = 1E8, spp2DisQuotaFine=1E8, simNumber=30, fuelUse=1, fuelPrice=40, gearMaintenance=2, landingCosts=0, increments=25, interpolationdistance=5, addNoFishing=TRUE, spp1DiscardSteps=0, spp2DiscardSteps=0, spp3DiscardSteps=0, spp4DiscardSteps=0, spp5DiscardSteps=0, choiceDist=0, numThreads=4){
    res <- new("DynState.control", spp1LndQuota= as.double(spp1LndQuota), spp2LndQuota=as.double(spp2LndQuota), spp1DisQuota= as.double(spp1DisQuota), spp2DisQuota=as.double(spp2DisQuota), spp1LndQuotaFine= as.double(spp1LndQuotaFine), spp2LndQuotaFine= as.double(spp2LndQuotaFine), spp1DisQuotaFine= as.double(spp1DisQuotaFine), spp2DisQuotaFine= as.double(spp2DisQuotaFine), simNumber=as.integer(simNumber), fuelUse=as.double(fuelUse), fuelPrice=as.double(fuelPrice), gearMaintenance=as.double(gearMaintenance), landingCosts=as.double(landingCosts), increments=as.integer(increments), interpolationdistance= as.integer(interpolationdistance), addNoFishing=addNoFishing, spp1DiscardSteps=as.double(spp1DiscardSteps), spp2DiscardSteps=as.double(spp2DiscardSteps), spp3DiscardSteps=as.double(spp3DiscardSteps), 
spp4DiscardSteps=as.double(spp4DiscardSteps), spp5DiscardSteps=as.double(spp5DiscardSteps), choiceDist=as.integer(choiceDist), numThreads=as.integer(numThreads))
   return(res)
}
...
annualFine <- function(dynstate){
    res <- ifelse(apply(spp1Landings(sim(dynstate)),2,sum) > control(dynstate)@spp1LndQuota,
           ...
           ifelse(apply(spp1Discards(sim(dynstate)),2,sum) > control(dynstate)@spp1DisQuota,
                 (apply(spp1Discards(sim(dynstate)),2,sum) - control(dynstate)@spp1DisQuota) *control(dynstate)@spp1DisQuotaFine ,0)+
           ifelse(apply(spp2Discards(sim(dynstate)),2,sum) > control(dynstate)@spp2DisQuota,
                 (apply(spp2Discards(sim(dynstate)),2,sum) - control(dynstate)@spp2DisQuota) *control(dynstate)@spp2DisQuotaFine ,0)
