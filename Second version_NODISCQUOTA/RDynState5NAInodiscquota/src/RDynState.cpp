...
float utility (int aLndSpp1,  int aLndSpp1Quota, float aSpp1LndQuotaFine, int aLndSpp2,  int aLndSpp2Quota, float aSpp2LndQuotaFine)
{                                                              
  float Fine1 = 0;
  float Fine2 = 0;
...
float FFF (int aLndSpp1,int aLndSpp2, int aNoInc, ATYPE aLndParmsAgg, ATYPE aDisParmsAgg, int aTime, int aNPatch, double aShortTermEcon[], FTYPE anFF1, ITYPE aIStar)
{      
 ...
 double rhs;
 double vMax = -2E120;
 int theLndSpp1Val, theLndSpp2Val;
  ...
  for (int i = 0; i < aNPatch; i++){
    ...
    if (rhs > vMax)  {
          vMax = rhs;
          aIStar[aLndSpp1][aLndSpp2] = i; 	/* Vmax is defined as a very small number */ 
  ...
  for (int s= 0; s < aSimNumber; s++)	 { /* go through the x simulations */   
      ...
      for (int t = 0; t < aHorizon; t++) { 
          ...
          // get best patch from the array
            ...
            int choicepos= sp2lnd + sp1lnd*maxspp[t]; /* creating an array of arrays */
...
void interpolateF ( FTYPE anFF1, int idis, int maxsp){
  ...
  #pragma omp parallel private(tdist1,tdist2,Lspp1,Lspp2)
  {
  ...
  for (Lspp1 = 0; Lspp1 < maxsp-1; Lspp1++) {
    for (Lspp2 = 0; Lspp2 < maxsp-1; Lspp2 ++) {
	  ...
	  anFF1[Lspp1][Lspp2] =
        (tdist1     * tdist2     * anFF1[(Lspp1/ idis)    * idis][(Lspp2/ idis)    * idis])+
	    ((1-tdist1) * tdist2     * anFF1[((Lspp1/idis) +1)* idis][(Lspp2/ idis)    * idis])+
	    (tdist1     * (1-tdist2) * anFF1[(Lspp1/ idis)    * idis][((Lspp2/idis) +1)* idis])+
	    ((1-tdist1) * (1-tdist2) * anFF1[((Lspp1/idis) +1)* idis][((Lspp2/idis) +1)* idis]);
    }
  }
  }
}
...
void interpolateI ( ITYPE aIStar, int idis, int maxsp){
  ...
  for (Lspp1 = 0; Lspp1 < maxsp-1; Lspp1++) {
    for (Lspp2 = 0; Lspp2 < maxsp-1; Lspp2 ++) {
	  ...
	  aIStar[Lspp1][Lspp2] =
            (tdist1     * tdist2     * aIStar[(Lspp1/ idis)    * idis][(Lspp2/ idis)    * idis])+
	    ((1-tdist1) * tdist2     * aIStar[((Lspp1/idis) +1)* idis][(Lspp2/ idis)    * idis])+
	    (tdist1     * (1-tdist2) * aIStar[(Lspp1/ idis)    * idis][((Lspp2/idis) +1)* idis])+
	    ((1-tdist1) * (1-tdist2) * aIStar[((Lspp1/idis) +1)* idis][((Lspp2/idis) +1)* idis]);
    }
  }
  }
}
...
Rprintf("Start of DynStateF\n");
  ...
  FTYPE theFF0              = (FTYPE) matalloc(sizeof(float), (void*) 0, 2, SPP1CAPACITY, SPP2CAPACITY);
  FTYPE theFF1              = (FTYPE) matalloc(sizeof(float), (void *)0, 2, kSpp1Capacity, kSpp2Capacity);
  ITYPE theIStar            = (ITYPE) matalloc(sizeof(UFINT), (void *)0, 2, kSpp1Capacity, kSpp2Capacity);
  ...
  int Lndspp1, Lndspp2, idisn, cntr, p;
  ...
  for (int t = kHorizon - 1; t >= 0; t--) {                                             // start the backward calculation
    ...
    for (p = 0; p < cntr; p++) {
	  ...
	  theFF0[Lndspp1][Lndspp2]= FFF(Lndspp1, Lndspp2, whatRangeLT[t], 
    ...
    for (Lndspp1 = 0; Lndspp1 < maxspp[t]; Lndspp1++) {
      for (Lndspp2 = 0; Lndspp2 < maxspp[t]; Lndspp2++) {
	          ...
	          ii++;
	            ...
	            if((theIStar[Lndspp1][Lndspp2] != val[t][ii-1]) || (rep[t][ii-1] > 65531)) {
	               ii++;
	               val[t].push_back(theIStar[Lndspp1][Lndspp2]);
	               rep[t].push_back(1);
	            } else {
...
#pragma omp parallel private(Lndspp1,Lndspp2)
    ...
    for (Lndspp1 = 0; Lndspp1 < maxspp[t]; Lndspp1++) {
      for (Lndspp2 = 0; Lndspp2 < maxspp[t]; Lndspp2++) {
               theFF1[Lndspp1][Lndspp2] =  theFF0[Lndspp1][Lndspp2];
