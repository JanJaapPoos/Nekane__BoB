...
dimnames<-list(len=c('1','2'), year=c(2003:2012),unit=c("OTB","PTB"),
...
dimnames<-list(len=c('1','2'),year="unique",unit="unique", 
...
for(i in 1:5){  
      ...
      #Remove area VII due to abscence of fishing activity during 2012
      catchMean(sp[[i]])<- catchMean(sp[[i]])[,,-2]
      ...
      #Remove area VII due to abscence of fishing activity during 2012
      catchSigma(sp[[i]])<- catchSigma(sp[[i]])[,,-2]
...
# Add a sigma of 0.1 of catchmean
catchSigma(hke) <- catchMean(hke)*0.1
catchSigma(mac) <- catchMean(mac)*0.1
catchSigma(hom) <- catchMean(hom)*0.1
catchSigma(lez) <- catchMean(lez)*0.1
catchSigma(others) <- catchMean(others)*0.1

save(CPUE_2012,catchmean_2012, mac, hke, hom, lez, others,
